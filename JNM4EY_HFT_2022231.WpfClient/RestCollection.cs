...
namespace JNM4EY_HFT_2022231.WpfClient
{
    public class RestService
    {
        ...
        public RestService(string baseurl, string pingableEndpoint = "swagger")
        {
            ...
            do
            {
                Uri uri = new Uri(new Uri(baseurl), pingableEndpoint);
                
                isOk = Ping(uri.AbsoluteUri);
            } while (isOk == false);
            Init(baseurl);
        }
        ...
        public async Task<ObservableCollection<T>> GetAsync<T>(string endpoint)
        {
            ObservableCollection<T> items = new ObservableCollection<T>();
            HttpResponseMessage response = await client.GetAsync(endpoint);
            if (response.IsSuccessStatusCode)
            {
                items = await response.Content.ReadAsAsync<ObservableCollection<T>>();
            }
            else
            {
        ...
        public ObservableCollection<T> Get<T>(string endpoint)
        {
            ObservableCollection<T> items = new ObservableCollection<T>();
    ...
    public class RestCollection<T> : INotifyCollectionChanged, IEnumerable<T>
    {
        ...
        public IEnumerator<T> GetEnumerator()
        {
            ...
            else return new ObservableCollection<T>().GetEnumerator();
        }
